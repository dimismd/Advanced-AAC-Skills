    ...
    <div className="aac-app">
      ...
      <div className="aac-board">
        {customCategories[activeCategory].symbols.map((symbol, idx) => (
          ...
          >
            {symbol.img.startsWith('/') ? (
              <img src={symbol.img} alt={symbol.text} className="aac-symbol-img" style={{width: '2.7rem', height: '2.7rem', objectFit: 'contain'}} />
            ) : (
              <span className="aac-symbol-img" role="img" aria-label={symbol.text}>{symbol.img}</span>
            )}
